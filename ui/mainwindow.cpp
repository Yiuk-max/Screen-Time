...
public:
  AppStatRow(const QIcon &icon, const QString &name,
             const QString &durationText, int seconds, int maxSeconds,
             QWidget *parent = nullptr)
      : QWidget(parent), m_icon(icon), m_name(name),
        m_durationText(durationText), m_seconds(seconds),
...
protected:
  void paintEvent(QPaintEvent *) override {
    ...
    const QRect iconRect(r.left(), r.center().y() - iconSize / 2, iconSize,
                         iconSize);
    if (!m_icon.isNull()) {
    ...
    const QRect nameRect(textLeft, r.top() + 10,
                         qMax(10, contentRight - textLeft), 18);
    p.drawText(
    ...
    const int filled =
        qBound(0, int(barBg.width() * double(m_seconds) / double(m_maxSeconds)),
               barBg.width());
    if (filled > 0) {
      p.setBrush(c.chart.bar);
      p.drawRoundedRect(
          QRect(barBg.left(), barBg.top(), qMax(3, filled), barBg.height()), 2,
          2);
    }
  }
...
MainWindow::MainWindow(Database *database, QWidget *parent)
  ...
  sidebarLayout->addWidget(m_sidebarToggleButton, 0,
  ...
  sidebarLayout->addWidget(m_settingsButton, 0,
  ...
  connect(m_contentStack, &QStackedWidget::currentChanged, this,
          [this](int index) {
            if (index == 0) {
              m_homeButton->setChecked(true);
              m_settingsButton->setChecked(false);
  ...
  connect(m_sidebarToggleButton, &QPushButton::clicked, this,
          [this]() { applySidebarMode(!m_sidebarExpanded); });
...
QWidget *MainWindow::createUsagePage() {
  ...
  m_hourlyChartWidget->setSizePolicy(QSizePolicy::Expanding,
  ...
  connect(m_usageSplitter, &QSplitter::splitterMoved, this,
          [this](int, int) { clampUsageSplitter(); });

  layout->addLayout(switchRow);
  layout->addWidget(m_usageSplitter, 1);

  connect(m_dailyButton, &QPushButton::clicked, this,
          &MainWindow::fillAppStatsForDaily);
  connect(m_weeklyButton, &QPushButton::clicked, this,
          &MainWindow::fillAppStatsForWeekly);

  m_refreshTimer.setInterval(5000);
  connect(&m_refreshTimer, &QTimer::timeout, this,
          &MainWindow::refreshUsageData);
...
QWidget *MainWindow::createSettingsPage() {
  ...
  connect(m_themeCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, [this](int index) {
            ...
            ThemeManager::instance().setTheme(
                m_themeCombo->itemData(index).toString());
          });
  ...
  rebuildAccentCombo();
  connect(m_accentCombo, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, [this](int index) {
            if (!m_accentCombo || index < 0)
              return;
            const AccentColor newAccent = static_cast<AccentColor>(
                m_accentCombo->itemData(index).toInt());
  ...
  const bool notifEnabled =
      settings.value(QStringLiteral("ui/notifications"), true).toBool();
  m_notificationSwitch->setChecked(notifEnabled);
  connect(m_notificationSwitch, &FluentToggleSwitch::toggled, this,
          [](bool checked) {
            QSettings s(QStringLiteral("ScreenTime"),
                        QStringLiteral("ScreenTime"));
  ...
  connect(m_autoStartSwitch, &FluentToggleSwitch::toggled, this,
          [this](bool checked) {
  ...
  auto *githubButton =
      new QPushButton(QStringLiteral("Yiuk-max/Screen-Time"), githubRow);
  githubButton->setCursor(Qt::PointingHandCursor);
  connect(githubButton, &QPushButton::clicked, this, []() {
    QDesktopServices::openUrl(
        QUrl(QStringLiteral("https://github.com/Yiuk-max/Screen-Time")));
  });
  ...
  QSettings apiSettings(QStringLiteral("ScreenTime"),
                        QStringLiteral("ScreenTime"));
  const QString savedApiKey =
      apiSettings.value(QStringLiteral("ai/deepseek_api_key")).toString();
  if (!savedApiKey.isEmpty()) {
    m_deepseekApiKeyEdit->setText(savedApiKey);
  }

  // 文本变化时立即保存，不必等 editingFinished（焦点丢失）
  connect(m_deepseekApiKeyEdit, &QLineEdit::textChanged, this,
  ...
  // 连接启用开关
  connect(m_aiReportEnabledSwitch, &FluentToggleSwitch::toggled, this,
          [this](bool checked) {
            QSettings s(QStringLiteral("ScreenTime"),
                        QStringLiteral("ScreenTime"));
  ...
  QSettings aiToggledSettings(QStringLiteral("ScreenTime"),
                              QStringLiteral("ScreenTime"));
  const bool aiEnabled =
      aiToggledSettings.value(QStringLiteral("ai/enabled"), false).toBool();
  ...
  makeHelpRow(
      "settings.about",
      QStringLiteral("Screen Time %1").arg(QStringLiteral(SCREENTIME_VERSION)),
      nullptr, [this]() {
        QMessageBox::about(
  ...
  makeHelpRow(
      "settings.repo", QStringLiteral("Yiuk-max/Screen-Time"), nullptr, []() {
        QDesktopServices::openUrl(
            QUrl(QStringLiteral("https://github.com/Yiuk-max/Screen-Time")));
      });
...
void MainWindow::onThemeChanged(const Theme &theme) {
  ...
  if (m_themeCombo) {
    const int index =
        m_themeCombo->findData(ThemeManager::instance().themeId());
  ...
  if (m_accentCombo) {
    const int index = m_accentCombo->findData(
        static_cast<int>(ThemeManager::instance().accent()));
...
void MainWindow::refreshIcons(const Theme &theme) {
  ...
  if (m_homeButton) {
    m_homeButton->setIcon(ThemeStyles::svgIcon(QString::fromUtf8(kHomeIconSvg),
                                               navColor, iconSize));
    m_homeButton->setIconSize(iconSize);
  }
  if (m_aiReportButton) {
    m_aiReportButton->setIcon(ThemeStyles::svgIcon(
        QString::fromUtf8(kAiIconSvg), navColor, iconSize));
    m_aiReportButton->setIconSize(iconSize);
  }
  if (m_settingsButton) {
    m_settingsButton->setIcon(ThemeStyles::svgIcon(
        QString::fromUtf8(kGearIconSvg), navColor, iconSize));
    m_settingsButton->setIconSize(iconSize);
  }
  if (m_shareButton) {
    m_shareButton->setIcon(ThemeStyles::svgIcon(
        QString::fromUtf8(kShareIconSvg), theme.colors.link, iconSize));
    m_shareButton->setIconSize(iconSize);
  }
  if (m_sidebarToggleButton) {
    m_sidebarToggleButton->setIcon(
        ThemeStyles::svgIcon(QString::fromUtf8(kMenuIconSvg),
                             theme.colors.textPrimary, QSize(20, 20)));
...
void MainWindow::rebuildThemeCombo() {
  ...
  for (const ThemeOption &option : ThemeManager::instance().themeOptions()) {
    ...
    const QString name =
        TranslationManager::instance().has(key) ? i18n(key) : option.name;
    m_themeCombo->addItem(name, option.id);
    const QString descKey = key + QStringLiteral(".desc");
    const QString description = TranslationManager::instance().has(descKey)
                                    ? i18n(descKey)
                                    : option.description;
    if (!description.isEmpty()) {
      m_themeCombo->setItemData(m_themeCombo->count() - 1, description,
                                Qt::ToolTipRole);
    }
  }
...
void MainWindow::refreshAppStatsList(
  ...
  for (const auto &item : sortedItems) {
    ...
    auto *widget = new AppStatRow(iconForApp(item.first), item.first,
                                  formatDuration(item.second), item.second,
                                  maxSeconds, m_appStatsList);
...
void MainWindow::refreshUsageData() {
  ...
  for (const UsageRecord &record : todayRecords) {
    ...
    const QDateTime tracked =
        QDateTime::fromString(record.trackedAt, "yyyy-MM-dd HH:mm:ss");
    if (!tracked.isValid()) {
      continue;
    }
  ...
  for (const UsageRecord &record : weeklyRecords) {
    ...
    if (!record.appPath.isEmpty() &&
        !m_appPathByName.contains(record.appName)) {
      m_appPathByName[record.appName] = record.appPath;
    }
  ...
  for (int i = 0; i < 7; ++i) {
    ...
    for (auto it = weeklyAppSecondsByDay[i].cbegin();
         it != weeklyAppSecondsByDay[i].cend(); ++it) {
  ...
  for (auto it = weeklyAppSeconds.cbegin(); it != weeklyAppSeconds.cend();
       ++it) {
...
void MainWindow::updateDailyChartAndSummary() {
  ...
  const int totalMinutes = std::accumulate(m_dailyMinutesByHour.cbegin(),
                                           m_dailyMinutesByHour.cend(), 0);
  m_primaryStatLabel->setText(
      i18n("stat.today_total").arg(formatDuration(totalMinutes * 60)));
  QStringList hourLabels;
  for (int i = 0; i < 24; ++i) {
    hourLabels.append(i18n("hour.label").arg(i));
  }
  m_hourlyChartWidget->setChartData(m_dailyMinutesByHour, hourLabels,
                                    m_dailyTopAppsByHour, m_appIconByName, true,
                                    60, 30);
}
...
void MainWindow::updateWeeklySummary() {
  ...
  const int totalMinutes = std::accumulate(m_weeklyMinutesByDay.cbegin(),
                                           m_weeklyMinutesByDay.cend(), 0);
  const int avgMinutes = m_weeklyMinutesByDay.isEmpty()
                             ? 0
                             : totalMinutes / m_weeklyMinutesByDay.size();
  m_primaryStatLabel->setText(i18n("stat.week_total")
                                  .arg(formatDuration(totalMinutes * 60),
                                       formatDuration(avgMinutes * 60)));
  ...
  m_hourlyChartWidget->setChartData(m_weeklyMinutesByDay, dayLabels,
                                    m_weeklyTopAppsByDay, m_appIconByName,
                                    false, dynamicMax, tick);
}
...
QString MainWindow::startupLaunchMode() const {
  QSettings settings(QStringLiteral("ScreenTime"),
                     QStringLiteral("ScreenTime"));
  return settings
      .value(QStringLiteral("startup/launch_mode"), QStringLiteral("tray"))
      .toString();
}
...
void MainWindow::hideToTray() {
  ...
  if (s.value(QStringLiteral("ui/notifications"), true).toBool()) {
    m_trayIcon->showMessage(QStringLiteral("Screen Time"),
...
void MainWindow::applySidebarMode(bool expanded) {
  ...
  const int targetWidth =
      expanded ? design.sidebarExpandedWidth : design.sidebarCollapsedWidth;
  ...
  connect(
      anim, &QPropertyAnimation::finished, this, [this, expanded, design]() {
        m_leftSidebar->setFixedWidth(expanded ? design.sidebarExpandedWidth
                                              : design.sidebarCollapsedWidth);
        ...
        if (expanded) {
          ...
          if (m_aiReportButton) {
            ...
            m_aiReportButton->setText(QStringLiteral("  ") +
                                      i18n("nav.report"));
          }
          for (QPushButton *button :
               {m_settingsButton, m_homeButton, m_aiReportButton}) {
            if (!button) {
              continue;
            }
